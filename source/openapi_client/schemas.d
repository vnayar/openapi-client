...
void writeSchemaFiles(OasDocument oasDocument, string targetDir, string packageName) {
  foreach (string schemaName, OasSchema schema; oasDocument.components.schemas) {
    ...
    generateModuleCode(targetDir, jsonSchema, schema, packageName);
  }
}
  ...
  with (buffer) {
    ...
    foreach (string schemaRef; getSchemaReferences(oasSchema)) {
      ...
      if (schemaName == jsonSchema.schemaName)
        continue;
      put("import ");
      put(getModuleNameFromSchemaName(packageName, schemaName));
      put(" : ");
...
void generateSchemaInnerClasses(
  ...
  if (schema.type == "object" && schema.properties !is null) {
    if (schema.additionalProperties.type == Json.Type.Undefined
      ...
      string className = (schema.title !is null) ? schema.title : defaultName;
      if (className is null)
        throw new Exception("Creating a Inner Class for property requires a title or default name!");
      buffer.put(prefix);
      buffer.put("static class " ~ className ~ " {\n");
      foreach (string propertyName, OasSchema propertySchema; schema.properties) {
        generateSchemaInnerClasses(buffer, propertySchema, prefix ~ "  ");
    ...
    generateSchemaInnerClasses(buffer, schema.anyOf[0], prefix);
  }
}
